...
import com.neurogrid.smartfm.results.LoginResult;
import com.nullwire.trace.ExceptionHandler;

public class LoginActivity extends Activity implements View.OnClickListener {

	public static String username(Activity activity) {
		return activity.getSharedPreferences(Main.PREF_REF, 0).getString(
				"username", null);
	}
	...
	public static boolean isNotLoggedIn(Activity activity) {
		String username = activity.getSharedPreferences(Main.PREF_REF, 0)
				.getString("username", "");
		...
		if (v.getId() == R.id.login || v.getId() == R.id.password) {
			...
			final Thread login = new Thread() {
				public void run() {
					...
					// (!this.isInterrupted())*/
					String dialog_message = "Please try agai later";
					try {
						LoginActivity.login_result = new LoginResult(
								Main.lookup.login(Main.transport, username,
										password));
					} catch (Exception e) {
						e.printStackTrace();
						dialog_message = e.getMessage();
					}
					if (LoginActivity.login_result != null
							&& LoginActivity.login_result.success()) {
						login(LoginActivity.this, username, password);
						Main.lookup.username = username;
						Main.lookup.password = password;
	...
	public static CreateListResult createList(String name, String description,
		...
		try {
			...
			// API key?
			String auth = username(activity) + ":" + password(activity);
